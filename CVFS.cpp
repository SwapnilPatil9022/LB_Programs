//CVFS(Custmised Virtual File System) Project
...
//file type
#define REGULAR 1  //two tye file regular and spec
#define SPECIAL 2

#define START 0  //Lseek position
#define CURRENT 1
#define END 2
...
typedef struct inode
{
	...
	int InodeNumber;
	int FileSize;      //2048
	int FileActualSize;  //file chya data chi size 
	int FileType;
	char *Buffer;   //file madhlya data cha address hold karto
	int LinkCount;   //shortcut (shortkt ne file tyar kiliya) to sadya vadnar pn nahi kami pn honar nahi to 1 rahnar
	int ReferenceCount;  //file kitivala open kilay tyach count honar ani file geliki count kami honar
	int permission;  // 1read  2write 3
	struct inode *next;
}INODE,*PINODE,**PPINODE;

typedef struct filetable
{
	int readoffset;   //kuthun read karayach tyach address
	int writeoffset;  //kuthun write karaych tycha address
	int count;
	int mode;  // 1  2  3  kuthlya mode madhe file open kelya 
	PINODE ptrinode;  //ptr to inode
}FILETABLE,*PFILETABLE;

typedef struct ufdt
{
	PFILETABLE ptrfiletable;  //ufdt ch ptr points to filetable
}UFDT;

//UFDTArr[0].ptrfiletable -> ptrinode ->FileSize;
UFDT UFDTArr[MAXINODE];

SUPERBLOCK SUPERBLOCKobj;  //superblock ch object
	...
	while(1)
	{
		...
		else if(count == 2)
		{
			...
			else if(strcmp(command[0],"write")== 0)  //write demo.txt
			{
					...
					if(ret == -1)
						printf("ERROR : Permission defined\n");
					if(ret == -2)
		...
		else if(count == 3)
		{
			...
			else if(strcmp(command[0],"read")== 0)  //read demo.txt 5
			{
				fd = GetFDFromName(command[1]);
				if(fd == -1)
				{
